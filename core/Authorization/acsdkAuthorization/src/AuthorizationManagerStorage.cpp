...
std::shared_ptr<AuthorizationManagerStorage> AuthorizationManagerStorage::create(
    ...
    if (!storage) {
        ACSDK_ERROR(LX(__func__).d("isStorageNull", !storage));
...
bool AuthorizationManagerStorage::initializeDatabase() {
    ...
    if (!openLocked()) {
        if (!m_storage->createDatabase() || !openLocked()) {
            ACSDK_ERROR(LX(__func__).d("reason", "createDatabaseFailed"));
            return false;
        }
    }
    ...
    if (!m_storage->tableExists(COMPONENT_NAME, AUTH_STATE_TABLE, &exists)) {
        ACSDK_ERROR(LX(__func__).d("reason", "checkTableExistenceFailed"));
        return false;
    }
...
bool AuthorizationManagerStorage::store(const std::string& adapterId, const std::string& userId) {
    ...
    if (valueContainer.size() != 0) {
        ACSDK_WARN(LX(__func__).d("reason", "tableNotEmpty"));
    }

    if (!m_storage->put(COMPONENT_NAME, AUTH_STATE_TABLE, AUTH_ADAPTER_ID_KEY, adapterId)) {
        ACSDK_ERROR(LX(__func__).d("reason", "storeAdapterIdFailed").d("adapterId", adapterId));

        return false;
    }

    if (!m_storage->put(COMPONENT_NAME, AUTH_STATE_TABLE, USER_ID_KEY, userId)) {
        ACSDK_ERROR(LX(__func__).d("reason", "storeUserIdFailed").d("userId", userId));
        return false;
    }
...
bool AuthorizationManagerStorage::load(std::string* adapterId, std::string* userId) {
    ...
    if (!adapterId || !userId) {
        ACSDK_ERROR(LX(__func__).d("reason", "nullptr").d("isAdapterIdNull", !adapterId).d("isUserIdNull", !userId));
    ...
    if (it == valueContainer.end()) {
        ACSDK_DEBUG0(LX(__func__).d("reason", "missingAuthAdapterId"));
        *adapterId = "";
    } else {
        *adapterId = it->second;
    }
    ...
    if (it == valueContainer.end()) {
        ACSDK_DEBUG0(LX(__func__).d("reason", "missingUserId"));
        *userId = "";
    } else {
        *userId = it->second;
    }

    ACSDK_DEBUG5(LX(__func__).d("authAdapterId", *adapterId).d("userId", *userId));

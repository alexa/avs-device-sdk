...
/// @private
static const std::string TAG{"OpenSSL::CryptoCodec"};
...
bool OpenSslCryptoCodec::processAAD(
    ...
    int outLen = (dataInEnd - dataInBegin) + cipherBlockSize;

    if (OPENSSL_OK == EVP_CipherUpdate(m_cipherCtx, nullptr, &outLen, &*dataInBegin, dataInEnd - dataInBegin)) {
        return true;
    } else {
...
bool OpenSslCryptoCodec::process(
    ...
    if (OPENSSL_OK == EVP_CipherUpdate(m_cipherCtx, &dataOut[index], &outLen, &*dataInBegin, dataInEnd - dataInBegin)) {
        dataOut.resize(index + outLen);
...
bool OpenSslCryptoCodec::getTag(Tag& tag) noexcept {
    ...
    if (OPENSSL_OK != EVP_CIPHER_CTX_ctrl(m_cipherCtx, EVP_CTRL_GCM_GET_TAG, tag.size(), tag.data() + tagOffset)) {
...
bool OpenSslCryptoCodec::setTag(const Tag& tag) noexcept {
    ...
    if (OPENSSL_OK != EVP_CIPHER_CTX_ctrl(m_cipherCtx, EVP_CTRL_GCM_SET_TAG, tag.size(), tagData)) {

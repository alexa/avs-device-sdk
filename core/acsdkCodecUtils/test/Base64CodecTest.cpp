...
#include <acsdkCodecUtils/Base64.h>
...
TEST(Base64CodecTest, test_base64EncodeEmpty) {
    std::string encoded;
...
TEST(Base64CodecTest, test_base64EncodeAppend) {
...
// Test encoding works
TEST(Base64CodecTest, test_base64EncodeTestStr) {
    std::string encoded;
...
TEST(Base64CodecTest, test_base64DecodeEmpty) {
    Bytes decoded;
...
TEST(Base64CodecTest, test_base64DecodeAppend) {
    Bytes decoded{1};
...
// Test decoding works
TEST(Base64CodecTest, test_base64DecodeTestStr) {
    Bytes decoded;
...
// Test decoding works and appends buffer
TEST(Base64CodecTest, test_base64DecodeAppendWhitespace) {
    Bytes decoded{1};
...
TEST(Base64CodecTest, test_base64DecodeError) {
    Bytes decoded;
    ASSERT_FALSE(decodeBase64("....", decoded));
}

// Test decoding fails on error
TEST(Base64CodecTest, test_base64DecodeErrorBadTail) {
    Bytes decoded;
    ASSERT_FALSE(decodeBase64("AA=C", decoded));
}

// Test decoding fails on error
TEST(Base64CodecTest, test_base64DecodeErrorDataAfterEnd) {
    Bytes decoded;
    ASSERT_FALSE(decodeBase64("AA==AAEC", decoded));
}

// Test decoding fails on error
TEST(Base64CodecTest, test_base64DecodeErrorEarlyEnd) {
    Bytes decoded;

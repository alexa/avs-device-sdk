...
TEST_F(HTTP2StreamPoolTest, GetStreamSendNullPtrForMoreThanMaxStreams) {
    ...
    for (int numberOfStreamSent = 0; numberOfStreamSent < TEST_MAX_STREAMS; numberOfStreamSent++) {
        stream = m_testableStreamPool->createGetStream(
                TEST_LIBCURL_URL, LIBCURL_TEST_AUTH_STRING, &m_testableConsumer);
...
TEST_F(HTTP2StreamPoolTest, initPostFails) {
    std::shared_ptr<HTTP2Stream> stream;
    stream = m_testableStreamPool->createPostStream("", LIBCURL_TEST_AUTH_STRING, m_mockMessageRequest, &m_testableConsumer);
    ASSERT_EQ(stream, nullptr);
    stream = m_testableStreamPool->createPostStream(TEST_LIBCURL_URL, "", m_mockMessageRequest, &m_testableConsumer);
    ASSERT_EQ(stream, nullptr);
    stream = m_testableStreamPool->createPostStream(TEST_LIBCURL_URL, LIBCURL_TEST_AUTH_STRING, nullptr, &m_testableConsumer);
    ASSERT_EQ(stream, nullptr);
    stream = m_testableStreamPool->createPostStream(TEST_LIBCURL_URL, LIBCURL_TEST_AUTH_STRING, m_mockMessageRequest, nullptr);
    ASSERT_EQ(stream, nullptr);
}
...
TEST_F(HTTP2StreamPoolTest, ReleaseStreamTestSendMoreThanMaxStreams) {
    ...
    for (int count = 0; count < TEST_MAX_STREAMS; count++) {
        stream_pool.push_back(m_testableStreamPool->createGetStream(
                TEST_LIBCURL_URL, LIBCURL_TEST_AUTH_STRING, &m_testableConsumer));

This is a preliminary release of the Alexa Voice Service (AVS) C++ SDK, which provides components for authentication and communications with the Alexa Voice Service (AVS), specifically AuthDelegate, the Alexa Communications Library (ACL), and associated APIs. This release does not include design documentation or support for message orchestration.  

## Overview

The Alexa Voice Service (AVS) Software Development Kit (SDK) for C++ provides a modern C++ (11 or later) interface for AVS that allows developers to add intelligent voice control to connected products. It is modular and abstracted, providing components to handle discrete functionality such as speech capture, audio processing, and communications, with each component exposing APIs that you can use and customize for your integration.
...
This architecture diagram illustrates the data flows between components that comprise the AVS C++ SDK.

![](avs-cpp-sdk-architecture.png)
...
**Audio Input Processor (AIP)** - Manages the captured audio streamed to AVS. Input sources may include: on-product microphones, remote microphones, etc. The AVS C++ SDK expects a single audio input from the AIP.
...
* `DEBUG` - Shows debug logs with `-g` compiler option.
* `RELEASE` - Removes debug logs from the build.
...
Unit tests for the AVS C++ SDK use the [Google Test](https://github.com/google/googletest) framework. Ensure that the [Google Test](https://github.com/google/googletest) is installed, then run the following command:
`make all test`
...
This is a preview of the SDK. The purpose is to provide an early view of the architecture and APIs. Not all functionality has been implemented or tested.

Functionality not yet implemented:

* Ping logic
* Reconnect logic
* Server-initiated disconnect logic

| Version | Release Date | Notes |
|---------|--------------|-------|
| v0.1 | 1/20/2017 | Out-of-source build is required. |
| v0.1 Pre-Release | 1/13/2017 | SDK requires libcurl 7.50.2 or later. Added instructions to quickly obtain test credentials. |
| v0.1 Pre-Release | 1/6/2017 | Updated terms and descriptions. |
| v0.1 Pre-Release | 12/30/2016 | Pre-release. |

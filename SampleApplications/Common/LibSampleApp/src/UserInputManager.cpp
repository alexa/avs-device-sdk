...
std::unique_ptr<UserInputManager> UserInputManager::create(
    ...
    if (!guiRenderer) {
        ACSDK_CRITICAL(LX("Invalid GUIRenderer passed to UserInputManager"));
        return nullptr;
    }
    ...
    return std::unique_ptr<UserInputManager>(
        new UserInputManager(interactionManager, guiRenderer, consoleReader, localeAssetsManager, defaultEndpointId));
}
...
SampleAppReturnCode UserInputManager::run() {
    ...
    while (true) {
        ...
        if (x == QUIT) {
            ...
            avsCommon::utils::Optional<bool> optionalState =
                m_guiRenderer->getGuiToggleState(GuiRenderer::TOGGLE_NAME_SHUFFLE);
            // Toggle the state if available, else set it to false to disable
            m_interactionManager->playbackShuffle(optionalState.hasValue() ? !optionalState.value() : false);
        } else if (x == LOOP) {
            avsCommon::utils::Optional<bool> optionalState =
                m_guiRenderer->getGuiToggleState(GuiRenderer::TOGGLE_NAME_LOOP);
            // Toggle the state if available, else set it to false to disable
            m_interactionManager->playbackLoop(optionalState.hasValue() ? !optionalState.value() : false);
        } else if (x == REPEAT) {
            avsCommon::utils::Optional<bool> optionalState =
                m_guiRenderer->getGuiToggleState(GuiRenderer::TOGGLE_NAME_REPEAT);
            // Toggle the state if available, else set it to false to disable
            m_interactionManager->playbackRepeat(optionalState.hasValue() ? !optionalState.value() : false);
        } else if (x == THUMBS_UP) {
            avsCommon::utils::Optional<bool> optionalState =
                m_guiRenderer->getGuiToggleState(GuiRenderer::TOGGLE_NAME_THUMBSUP);
            // Toggle the state if available, else set it to false to disable
            m_interactionManager->playbackThumbsUp(optionalState.hasValue() ? !optionalState.value() : false);
        } else if (x == THUMBS_DOWN) {
            avsCommon::utils::Optional<bool> optionalState =
                m_guiRenderer->getGuiToggleState(GuiRenderer::TOGGLE_NAME_THUMBSDOWN);
            // Toggle the state if available, else set it to false to disable
            m_interactionManager->playbackThumbsDown(optionalState.hasValue() ? !optionalState.value() : false);

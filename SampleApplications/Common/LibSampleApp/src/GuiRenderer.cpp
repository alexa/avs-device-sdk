...
void GuiRenderer::renderPlayerInfoCard(
    ...
    buffer += "# Offset milliseconds : " + std::to_string(info.mediaProperties->getAudioItemOffset().count()) + "\n";
    buffer += RENDER_FOOTER;
    buffer += jsonPayload + "\n";
...
avsCommon::utils::Optional<bool> GuiRenderer::getGuiToggleState(const std::string& toggleName) {
    const auto& it = m_guiToggleStateMap.find(toggleName);
    return it != m_guiToggleStateMap.end() ? it->second : avsCommon::utils::Optional<bool>();
}

 ...
 * @return True if the storage component's underlying database is opened, false
 * otherwise.
 */
...
/**
 * Test that empty database (due to a corruption or crash) results in default
 * indicator state being used (non-undefined).
 */
...
/**
 * Test that invalid database value (due to a corruption or crash) results in
 * default indicator state being used (non-undefined).
 */
...
TEST_F(NotificationsStorageTest, test_queueOrder) {
    ...
    // generate ints with a random generator, this will be used to produce random
    // bool values
    ...
    // dequeue all the NotificationIndicators and check that they match the random

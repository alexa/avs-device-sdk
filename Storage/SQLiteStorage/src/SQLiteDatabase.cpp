...
bool SQLiteDatabase::initialize() {
    if (m_dbHandle) {
        ACSDK_ERROR(LX("initializeFailed").m("Database is already open."));
        return false;
    }

    if (avsCommon::utils::file::fileExists(m_storageFilePath)) {
        ACSDK_ERROR(LX("initializeFailed").m("File specified already exists.").d("file path", m_storageFilePath));
        return false;
    }

    m_dbHandle = createSQLiteDatabase(m_storageFilePath);
    if (!m_dbHandle) {
        ACSDK_ERROR(LX("initializeFailed").m("Database could not be created.").d("file path", m_storageFilePath));
        return false;
    }
...
bool SQLiteDatabase::open() {
    if (m_dbHandle) {
        ACSDK_ERROR(LX("openFailed").m("Database is already open."));
        return false;
    }

    if (!avsCommon::utils::file::fileExists(m_storageFilePath)) {
        ACSDK_DEBUG0(LX("openFailed").m("File specified does not exist.").d("file path", m_storageFilePath));
        return false;
    }

    m_dbHandle = openSQLiteDatabase(m_storageFilePath);
    if (!m_dbHandle) {
        ACSDK_ERROR(LX("openFailed").m("Database could not be opened.").d("file path", m_storageFilePath));
        return false;
    }
...
bool SQLiteDatabase::tableExists(const std::string& tableName) {
    if (!alexaClientSDK::storage::sqliteStorage::tableExists(m_dbHandle, tableName)) {
        ACSDK_DEBUG0(LX("tableExistsFailed")
                         .d("reason", "table doesn't exist or there was an error checking")
                         .d("table", tableName));
        return false;
    }
    return true;
...
bool SQLiteDatabase::clearTable(const std::string& tableName) {
    if (!alexaClientSDK::storage::sqliteStorage::clearTable(m_dbHandle, tableName)) {
        ACSDK_ERROR(LX("clearTableFailed").d("could not clear table", tableName));
        return false;
    }
...
bool SQLiteDatabase::dropTable(const std::string& tableName) {
    if (!alexaClientSDK::storage::sqliteStorage::dropTable(m_dbHandle, tableName)) {
        ACSDK_ERROR(LX("dropTableFailed").d("could not drop table", tableName));
        return false;
    }
...
void SQLiteDatabase::close() {
    if (m_dbHandle) {
        if (m_transactionIsInProgress) {
            ACSDK_ERROR(LX("closeError").d("reason", "There is an incomplete transaction. Rolling it back."));
            rollbackTransaction();
        }

...
#ifndef ALEXA_CLIENT_SDK_AVSCOMMON_INCLUDE_AVSCOMMON_UTILS_TIMING_TIMER_H_
#define ALEXA_CLIENT_SDK_AVSCOMMON_INCLUDE_AVSCOMMON_UTILS_TIMING_TIMER_H_
...
bool Timer::start(
    ...
    if (delay < std::chrono::duration<Rep, Period>::zero() || period < std::chrono::duration<Rep, Period>::zero()) {
        avsUtils::Logger::log("Unable to start timer: negative delay or period.");
        return false;
    }

    // Can't start if already running.
    if (!activate()) {
        avsUtils::Logger::log("Unable to start timer: already running.");
        return false;
    }
...
#endif //ALEXA_CLIENT_SDK_AVSCOMMON_INCLUDE_AVSCOMMON_UTILS_TIMING_TIMER_H_
